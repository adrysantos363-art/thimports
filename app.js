...
$("loginBtn").addEventListener("click",async()=>{
  ...
  try{
    ...
    if(cred.user.uid!==ADMIN_UID){await firebaseAuth.signOut();throw new Error("Este usuário não possui permissão de administrador.");}
    $("loginError").textContent="";closeModal("loginModal");openModal("adminModal");renderAdmin();loadSettings();
    setFirebaseStatus("Administrador conectado. Sincronização ativa.");
  }catch(error){console.error(error);$("loginError").textContent="Login Firebase: "+error.message;}
});
...
window.deleteProduct=async id=>{if(confirm("Excluir este produto?")){products=products.filter(p=>p.id!==id);saveProducts();renderProducts();renderAdmin();if(firestoreDb && firebaseAuth?.currentUser?.uid===ADMIN_UID){try{await firestoreDb.collection("products").doc(String(id)).delete();setFirebaseStatus("Produto excluído do Firebase.");}catch(error){setFirebaseStatus("Falha ao excluir: "+error.message,true);}}}};
$("productForm").addEventListener("submit",e=>{
  ...
  if(firestoreDb && firebaseAuth?.currentUser?.uid===ADMIN_UID){
    saveProductToFirebase(data.id);
  }else{
    setFirebaseStatus("Produto salvo neste navegador. Entre com o Firebase para sincronizar.",true);
  }
  resetForm();
});
...
const PUBLIC_FIREBASE_PROJECT_ID = "thimports-1ce81";
...
function firestoreRestValue(v){
  ...
  if(v.mapValue) return firestoreRestMap(v.mapValue.fields || {});
  if(v.arrayValue) return (v.arrayValue.values || []).map(firestoreRestValue);
  return null;
}
function firestoreRestMap(fields){
  const out={};
  Object.entries(fields || {}).forEach(([k,v])=>out[k]=firestoreRestValue(v));
  return out;
}
async function fetchPublicFirestoreCollection(collection){
  const url=`https://firestore.googleapis.com/v1/projects/${PUBLIC_FIREBASE_PROJECT_ID}/databases/(default)/documents/${collection}?pageSize=100`;
  const response=await fetch(url,{cache:"no-store"});
  if(!response.ok) throw new Error(`HTTP ${response.status}`);
  return response.json();
}
async function syncPublicStore(){
  try{
    const data=await fetchPublicFirestoreCollection("products");
    const remoteProducts=(data.documents || []).map(doc=>firestoreRestMap(doc.fields || {})).filter(p=>p.id!==undefined);
    if(remoteProducts.length){
      const normalized=remoteProducts.map(p=>({...p,id:Number(p.id),price:Number(p.price)}));
      const changed=JSON.stringify(normalized)!==JSON.stringify(products);
      products=normalized;
      saveProducts();
      if(changed){renderProducts();renderAdmin();renderCart();}
    }
    try{
      const settingsUrl=`https://firestore.googleapis.com/v1/projects/${PUBLIC_FIREBASE_PROJECT_ID}/databases/(default)/documents/store/settings`;
      const sr=await fetch(settingsUrl,{cache:"no-store"});
      if(sr.ok){
        const sd=await sr.json();
        if(sd.fields){
          const remote=firestoreRestMap(sd.fields);
          settings={...settings,...remote};
          saveSettings();
          renderCart();
        }
      }
    }catch(e){ console.warn("Configuração pública não carregada:",e); }
  }catch(error){
    console.warn("Sincronização pública do Firebase indisponível:",error);
  }
}
function startPublicSync(){
  syncPublicStore();
  if(!publicSyncTimer) publicSyncTimer=setInterval(syncPublicStore,PUBLIC_SYNC_INTERVAL_MS);
}
...
async function saveProductToFirebase(id){
  if(!firestoreDb){setFirebaseStatus("Conecte o Firebase primeiro.",true);return;}
  if(firebaseAuth?.currentUser?.uid!==ADMIN_UID){setFirebaseStatus("Faça login com o administrador do Firebase.",true);return;}
  const product=products.find(p=>p.id===id);
  if(!product)return;
  try{await firestoreDb.collection("products").doc(String(id)).set(product,{merge:true});setFirebaseStatus("Produto "+product.name+" salvo no Firebase.");}
  catch(error){console.error(error);setFirebaseStatus("Falha ao salvar produto: "+error.message,true);}
}
window.saveProductToFirebase=saveProductToFirebase;

let productsListenerStarted=false;
function startRealtimeProducts(){
  if(!firestoreDb || productsListenerStarted)return;
  productsListenerStarted=true;
  firestoreDb.collection("products").onSnapshot(snapshot=>{
    if(snapshot.empty)return;
    products=snapshot.docs.map(doc=>doc.data());
    saveProducts();renderProducts();renderAdmin();
    setFirebaseStatus("Produtos sincronizados automaticamente com o Firebase.");
  },error=>setFirebaseStatus("Sincronização: "+error.message,true));
}

...
async function signInAdmin(){
  ...
  try{
    ...
    if(!validFirebaseConfig(config)){
      $("loginError").textContent="Configure e salve o Firebase primeiro.";
      return;
    }
    if(typeof firebase==="undefined" || !firebase.auth){
      $("loginError").textContent="SDK de autenticação não carregado.";
      return;
    }
    if(!firebase.apps.length) firebase.initializeApp(config);
    const credential=await firebase.auth().signInWithEmailAndPassword(email,password);
    if(credential.user.uid!=="DqPq8jyUcNOfWrHYFWgssKHgCh93"){
      await firebase.auth().signOut();
      $("loginError").textContent="Este usuário não tem permissão de administrador.";
      return;
    }
    ...
    renderAdmin();
    loadSettings();
    setFirebaseStatus("Administrador autenticado no Firebase.");
  }catch(error){
    console.error(error);
    $("loginError").textContent="Falha no login: "+(error.code==="auth/invalid-credential"?"e-mail ou senha inválidos.":error.message);
  }
}
$("loginBtn").addEventListener("click",signInAdmin);
$("adminPassword").addEventListener("keydown",e=>{if(e.key==="Enter")signInAdmin()});
$("adminEmail").addEventListener("keydown",e=>{if(e.key==="Enter")signInAdmin()});
$("logoutBtn").addEventListener("click",async()=>{
  if(typeof firebase!=="undefined" && firebase.auth) await firebase.auth().signOut().catch(console.error);
...
$("uploadFirebase").addEventListener("click",async()=>{
  ...
  try{
    ...
    if(!currentUser){
      setFirebaseStatus("Faça login com Firebase Authentication antes de enviar.",true);
      return;
    }
    if(currentUser.uid!=="DqPq8jyUcNOfWrHYFWgssKHgCh93"){
      setFirebaseStatus("Usuário autenticado sem permissão de administrador.",true);
      return;
    }
